...
const SortingOptions = ({ dropdownValues, sort }: Props) => {
  const pathname = usePathname();
  const router = useRouter();
  const searchParams = useSearchParams();

  const [sortOption, setSortOption] = useState(
    sort ? sort : dropdownValues[2] ? dropdownValues[2] : "ascending"
  );
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant='outline'>
          {sortOption == "ascending" && dropdownValues[0]}{" "}
          {sortOption == "descending" && dropdownValues[1]}{" "}
      ...
      <DropdownMenuContent className=''>
        ...
        >
          <DropdownMenuRadioItem value='ascending'>
            <span className='font-medium'>{dropdownValues[0]}</span>
          </DropdownMenuRadioItem>
          <DropdownMenuRadioItem value='descending'>
            <span className='font-medium'>{dropdownValues[1]}</span>
          </DropdownMenuRadioItem>
        </DropdownMenuRadioGroup>
      </DropdownMenuContent>
    </DropdownMenu>
